...
(function(exports) {
   ...
   * @param {healthReportKey} healthReportKey
  ...
  function ContentSearchUIController(inputElement, tableParent, healthReportKey,
    ...
    gNewTab.registerListener("NewTab:ContentSearchService",
      this._onContentSearchService.bind(this));
  ...
  ContentSearchUIController.prototype = {
    ...
    set defaultEngine(engine) {
      ...
      this._defaultEngine = {
        name: engine.name,
        icon: icon,
        preconnectOrigin: engine.preconnectOrigin,
      };
    ...
    deleteSuggestionAtIndex: function(idx) {
      ...
      if (this.isFormHistorySuggestionAtIndex(idx)) {
        ...
        gNewTab.sendToBrowser("NewTab:RemoveFormHistoryEntry", suggestionStr);
    ...
    _onCommand: function(aEvent) {
      if (this.selectedButtonIndex === this._oneOffButtons.length) {
        ...
        gNewTab.sendToBrowser("NewTab:ManageEngines");
        return;
      }
    ...
    search: function(aEvent) {
      ...
      if (this.suggestionAtIndex(this.selectedIndex)) {
        eventData.selection = {
          ...
          kind: aEvent instanceof MouseEvent ? "mouse" : aEvent instanceof KeyboardEvent ? "key" : undefined,
        };
      }
    ...
    _cycleCurrentEngine: function(aReverse) {
      ...
      gNewTab.sendToBrowser("NewTab:SetCurrentEngine", engineName);
    },
    ...
    _onContentSearchService: function(message) {
      let methodName = "_onMsg" + message.name;
      if (methodName in this) {
        this[methodName](message);
      }
    },

    _onMsgFocusInput: function() {
      this.input.focus();
    },

    _onMsgSuggestions: function(message) {
      ...
      if (this._stickyInputValue !== suggestion.searchString ||
        this.defaultEngine.name !== suggestion.engineName) {
        return;
      }
      ...
      // Add the suggestions to the table. Max displayed suggestions is 6
      let searchWords =
        new Set(suggestion.searchString.trim().toLowerCase().split(/\s+/));
      for (let i = 0; i < 6; i++) {
        let type;
        let idx;
        if (i < suggestion.formHistory.length) {
          [type, idx] = ["formHistory", i];
        } else {
          let j = i - suggestion.formHistory.length;
          if (j < suggestion.remote.length) {
            [type, idx] = ["remote", j];
          } else {
            break;
          }
        ...
        this._suggestionsList.appendChild(
          this._makeTableRow(type, suggestion[type][idx], i, searchWords));
      }
    ...
    _onMsgState: function(message) {
      this.engines = message.state.engines;
      // No point updating the default engine (and the header) if there's no change.
      if (this.defaultEngine &&
        this.defaultEngine.name === message.state.currentEngine.name &&
        this.defaultEngine.icon === message.state.currentEngine.icon) {
        return;
      }
      this.defaultEngine = message.state.currentEngine;
    },
    ...
    _onMsgCurrentEngine: function(message) {
      this.defaultEngine = message.engine;
      this._pendingOneOffRefresh = true;

      // remove the preconnector if it exists, since the default engine changed
      // and a new speculative connect needs to be made.
      let exists = document.getElementById("preconnector") ? true : false;
      if (exists) {
        this._removePreconnector();
      }
    },

    _onMsgStrings: function(message) {
      this._strings = message.strings;
      if (!this.defaultEngine) {
        return;
      }
      this._updateDefaultEngineHeader();
    ...
    _removePreconnector: function() {
      let preconnector = document.getElementById("preconnector");
      preconnector.parentNode.removeChild(preconnector);
    },
    ...
    _speculativeConnect: function() {
      if (this.defaultEngine) {
        ...
        preConnector.crossorigin = "anonymous";
        preConnector.href = this.defaultEngine.preconnectOrigin;
    ...
    _makeTableRow: function(type, suggestionStr, currentRow, searchWords) {
      ...
      entry.id = this._idPrefix + "searchSuggestion" + currentRow;
    ...
    _getSearchEngines: function() {
      gNewTab.sendToBrowser("NewTab:GetState");
    },

    _getStrings: function() {
      gNewTab.sendToBrowser("NewTab:GetStrings");
    },

    _getSuggestions: function() {
      this._stickyInputValue = this.input.value;
      if (this.defaultEngine) {
        gNewTab.sendToBrowser("NewTab:GetSuggestions", {
    ...
    _makeTable: function(id) {
      ...
      button.textContent = "Change Search Settings";
      ...
      for (let i = 0; i < engines.length; ++i) {
        ...
        if (engine.iconBuffer) {
          ...
          uri = this._getImageURIForCurrentResolution(
            "images/search-engine-placeholder.png");
        }
  ...
  exports.ContentSearchUIController = ContentSearchUIController;
}(window));
