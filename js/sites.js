...
(function(exports) {
  ...
  Site.prototype = {
    ...
    _updateAttributes: function(aPinned) {
      ...
      if (aPinned) {
        this.node.data.pinned = true;
        control.data.title = gNewTab.newTabString("unpin");
      } else {
        delete this.node.data.pinned;
        control.data.title = gNewTab.newTabString("pin");
      }
    },
    ...
    _render() {
      ...
      link.data.title = tooltip;
      link.data.href = url;
      this._querySelector(".newtab-title").textContent = title;
      this.node.data.type = this.link.type;

      if (this.link.targetedSite) {
        if (this.node.getAttribute("data-type") !== "sponsored") {
          this._querySelector(".newtab-sponsored").textContent =
            gNewTab.newTabString("suggested.tag");
        }
        this.node.data.suggested = true;
    ...
    _ignoreHoverEvents(element) {
      element.addEventListener("mouseover", () => {
        this.cell.node.data.ignorehover = true;
      });
      element.addEventListener("mouseout", () => {
        delete this.cell.node.data.ignorehover;
      });
    },
    ...
    _toggleLegalText(buttonClass, explanationTextClass) {
      ...
      if (button.hasAttribute("data-active")) {
        ...
        let linkType = this.node.getAttribute("data-type");
        let isAffiliate = linkType === "affiliate";
        let isSuggested = this.node.getAttribute("data-suggested");
